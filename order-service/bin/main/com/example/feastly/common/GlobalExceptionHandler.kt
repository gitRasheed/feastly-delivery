...
class GlobalExceptionHandler {
    ...
    fun handleRestaurantNotAccepting(ex: RestaurantNotAcceptingOrdersException): ResponseEntity<ErrorResponse> =
        ResponseEntity.status(HttpStatus.CONFLICT)
            .body(ErrorResponse(
                error = ex.message ?: "Restaurant not accepting orders",
                internalDetails = mapOf(
                    "reason" to ex.reason,
                    "nextChangeAt" to ex.nextChangeAt?.toString()
                )
            ))
}
